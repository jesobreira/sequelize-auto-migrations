...
const parseIndex = function(idx) 
{
    ...
    if (idx.unique)
        options.type = options.indicesType = idx.name === 'PRIMARY' ? 'primary key' : 'UNIQUE';
            ...
            case 'N':
            {
                ...
                if (df.path.length === 1)
                {
                    ...
                    if (df.rhs.indexes)
                        ...
                        {
                          if (df.rhs.indexes[_i].name === 'PRIMARY') continue;
            ...
            case 'E':
            {
                ...
                if (df.path[1] === 'indexes')
                {
                    ...
                    for (let k in keys) {
                        ...
                        if (df.rhs[key].name === 'PRIMARY') continue;
...
let res = `{ fn: "addConstraint", params: [
    "${action.tableName}",
    ${JSON.stringify({...action.options, fields: action.fields})}
] }`;
                commandsUp.push(res);
                
                let nameOrAttrs = (action.options && action.options.indexName && action.options.indexName != '') ? `"${action.options.indexName}"` : JSON.stringify(action.fields);
                consoleOut.push(`addConstraint ${nameOrAttrs} to table "${action.tableName}"`);
            }
            break;
